...
BLOB_CONNECTION_STRING = os.getenv("BlobStorageConnectionString")
...
TWILIO_WHATSAPP_FROM = os.getenv("TWILIO_WHATSAPP_FROM")  # Twilio's WhatsApp sandbox number
...
@app.blob_trigger(arg_name="myblob", path="image-upload/{name}", connection="BlobStorageConnectionString")
def blob_trigger_iot(myblob: func.InputStream):
    try:
        ...
        # **Switched Labels:**
        predicted_label = 'Intruder' if confidence > 0.5 else 'Resident'

        # Get current timestamp in UTC
        timestamp = datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S UTC')

        # Log the result
        logging.info(f"Processed image: {myblob.name} - Prediction: {predicted_label} - Confidence: {confidence:.4f} - Timestamp: {timestamp}")
        ...
        recipients = [
            "whatsapp:+491621561715",  
            "whatsapp:+4915213037612",
            "whatsapp:+4915560768424",
            "whatsapp:+919904518345",  
        ]
        
        # **WhatsApp Message Handling**
        if predicted_label == "Intruder":
            message_body = (f"🚨 *SECURITY ALERT!* 🚨\n"
                            f"🔴 *Intruder Detected!*\n"
                            f"📷 Image: {myblob.name}\n"
            ...
            message_body = (f"✅ *Resident Detected*\n"
                            f"📷 Image: {myblob.name}\n"
                            f"📊 Confidence: {confidence:.4f}\n"
                            f"🕒 Timestamp: {timestamp}\n"
                            f"No action needed.")
        ...
        send_whatsapp_message(message_body, recipients, media_url=[sas_url])  
